...
//			       int zero)
int pram_new_data_block(struct inode *inode, unsigned long *blocknr,int zero)
{
...
//				   loff_t end)
void __pram_truncate_blocks(struct inode *inode, loff_t start,loff_t end)
{
	struct super_block *sb = inode->i_sb;
	struct pram_inode *pi = pram_get_inode(sb, inode->i_ino);
	int N = sb->s_blocksize >> 3; /* num block ptrs per block */
...
int pram_alloc_blocks(struct inode *inode, int file_blocknr, unsigned int num)
{
	...
	//printk(KERN_DEBUG "alloc;00\n");//
	if (!pi->i_type.reg.row_block) {
		/* alloc the 2nd order array block */

		//printk(KERN_DEBUG "alloc;01\n");//
	...
	for (i = first_row_index; i <= last_row_index; i++) {
		...
		for (j = first_col_index; j <= last_col_index; j++) {
			...
			//printk(KERN_DEBUG "alloc;06\n");

			if (!col[j]) {

				//printk(KERN_DEBUG "alloc;07\n");
	...
	errval = 0;
	//printk(KERN_DEBUG "alloc;08\n");
 fail:
	//printk(KERN_DEBUG "alloc;09\n");
	return errval;
}
...
/*
int pram_get_block_write(struct inode *inode, sector_t iblock, struct buffer_head *bh_result, int create){
	int ret = 1;
	printk(KERN_DEBUG "pram_get_block_write\n");
	if (ret > 0) {
...
static int pram_writepage(struct page *page, struct writeback_control *wbc){
	printk(KERN_DEBUG "pram_writepage\n");
	return block_write_full_page(page,pram_get_block_write,wbc);
}

static int pram_writepages(struct address_space *mapping, struct writeback_control *wbc){
	printk(KERN_DEBUG "pram_writepages!!!\n");
	return mpage_writepages(mapping, wbc, pram_get_block_write);
}

static int pram_write_begin(struct file *file, struct address_space *mapping,
		loff_t pos, unsigned len, unsigned flags, struct page **pagep, void **fsdata){
	int ret;
	ret = block_write_begin(mapping,pos,len,flags,pagep,pram_get_block_write);
	printk(KERN_DEBUG "pram_write_begin\n");
	return ret;
}

static int pram_write_end(struct file *file, struct address_space *mapping,
		loff_t pos, unsigned len, unsigned copied, struct page *page, void *fsdata){
	int ret;
	printk(KERN_DEBUG "pram_write_end\n");
...
const struct address_space_operations pram_aops = {
	...
	//.writepages	= pram_writepages,//
	//.write_begin	= pram_write_begin,//
	//.write_end	= pram_write_end,//
